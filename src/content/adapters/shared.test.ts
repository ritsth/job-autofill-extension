...
describe('isComboboxLike — dropdowns are never autofilled or given an AI-answer button', () => {
...
describe('RULES — the remaining field rules, against realistic concatenated labels', () => {
  ...
  it('no rule opts a dropdown into autofill', () => {
    // "Fill this page" never touches a <select> (see applyStandardFills, which
    // does not even query one). Rules used to carry a `selectOk` flag that let
    // four of them — state, country, work authorization, sponsorship — fill a
    // dropdown; that escape hatch is gone. A rule may only carry `test` and
    // `value`, so no rule can grow a silent replacement for it.
    for (const rule of RULES) {
      expect(Object.keys(rule).sort()).toEqual(['test', 'value']);
    }
  });
